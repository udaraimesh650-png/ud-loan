...
export const LoanCalculatorScreen: React.FC<LoanCalculatorScreenProps> = ({
  ...
  // ------------------------------------------------------------
  // BASE MAXIMUM LOAN PERIOD
  // ------------------------------------------------------------
  const baseMaxMonths = useMemo(() => {
    if (!selectedLoanType) {
      return 360;
    }
  ...
  // ------------------------------------------------------------
  // EFFECTIVE MAXIMUM PERIOD
  // Loan condition maximum vs age 60 maximum
  // ------------------------------------------------------------
  const effectiveMaxMonths = useMemo(() => {
    if (age60RemainingMonths === null) {
      return baseMaxMonths;
    }

    return Math.min(
      baseMaxMonths,
      Math.max(0, age60RemainingMonths)
    );
  }, [
    baseMaxMonths,
  ...
  const tenureMax = useMemo(() => {
    if (
      ...
      return Math.floor(
        effectiveMaxMonths / 12
      );
    }

    return effectiveMaxMonths;
  }, [
    effectiveMaxMonths,
    tenureUnit,
  ]);
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="mt-2 flex flex-col gap-1.5">
            ...
            {birthday &&
              age60RemainingMonths ===
                0 && (
                <div className="text-xs font-bold text-amber-700 px-2">
                  Age 60 reached. No
                  repayment period is
                  available.
                </div>
