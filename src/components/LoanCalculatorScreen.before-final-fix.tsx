...
// ------------------------------------------------------------
const getMonthsUntilAge60 = (birthdayValue: string): number | null => {
  ...
  if (today >= age60Date) return 0;
  let months = (age60Date.getFullYear() - today.getFullYear()) * 12 + (age60Date.getMonth() - today.getMonth());
  if (age60Date.getDate() < today.getDate()) months -= 1;
  return Math.max(0, months);
};

const age60RemainingMonths = useMemo(() => getMonthsUntilAge60(birthday), [birthday]);
...
if (selectedLoanType === 'guarantee-loan') {
  ...
  if (totalMonths > limits.maxPeriodMonths) {
    return `à·ƒà·à¶¸à·à¶¢à·’à¶šà¶­à·Šà·€ à¶šà·à¶½à¶º à¶…à¶±à·”à·€ à¶‹à¶´à¶»à·’à¶¸ à¶†à¶´à·ƒà·” à¶œà·™à·€à·“à¶¸à·š à¶šà·à¶½à¶º à¶¸à·à·ƒ ${limits.maxPeriodMonths} à¶šà·’.`;
  }
...
const tenureMax = useMemo(() => {
  if (!selectedLoanType) {
    return tenureUnit === 'years' ? 30 : 360;
  }

  if (selectedLoanType === 'guarantee-loan') {
    const maxMonths = guaranteeLimits?.maxPeriodMonths ?? 360;
  ...
  const maxMonths =
    conditions[selectedLoanType]?.maxPeriodMonths ?? 360;
  ...
  <div className="w-full max-w-[360px] mx-auto flex flex-col gap-4 py-4 z-10">
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="mt-2 flex flex-col gap-1.5">
        <label className="text-sm font-black text-slate-800 tracking-wide flex items-center gap-1.5 flex-wrap">
          <span className="w-2 h-2 rounded-full bg-slate-900 inline-block" />
          <span>BIRTHDAY</span>
          <span className="text-xs font-semibold text-slate-500 normal-case">(Optional)</span>
        </label>
        <input id="input-birthday" type="date" value={birthday} max={new Date().toISOString().split("T")[0]} onChange={(e) => setBirthday(e.target.value)} className="w-full py-3 px-5 rounded-full bg-white text-slate-900 font-bold text-base border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-400" />
        {birthday && age60RemainingMonths !== null && age60RemainingMonths > 0 && (
          <div className="text-xs font-bold text-sky-700 px-2">Age 60 remaining: {age60RemainingMonths} Months</div>
        )}
      </div>
    ...
    <div className="flex flex-col gap-1.5">
      <label className="text-sm font-black text-slate-800 tracking-wide flex items-center gap-1.5 flex-wrap">
        ...
        <span className="text-xs font-semibold text-slate-500 normal-case">
          (à·€à·à¶»à·Šà·‚à·’à¶š à¶´à·œà¶½à·“ à¶…à¶±à·”à¶´à·à¶­à·’à¶šà¶º)
        </span>
      </label>
