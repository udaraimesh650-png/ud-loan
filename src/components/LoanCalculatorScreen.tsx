...
if (found.defaultPeriodMonths) {
  const months = found.defaultPeriodMonths;
  ...
  if (
    ...
    setTenureValue((months / 12).toString());
    setTenureUnit('years');
  } else {
    setTenureValue(months.toString());
    setTenureUnit('months');
...
if (birthday && monthsUntilAge60 === null) {
  return 'Please enter a valid Birthday.';
}

if (birthday && monthsUntilAge60 !== null && monthsUntilAge60 <= 0) {
  return 'Loan repayment period is not available at age 60 or above.';
}

if (
  birthday &&
  monthsUntilAge60 !== null &&
  totalMonths > monthsUntilAge60
) {
  return `Maximum repayment period until age 60 is ${monthsUntilAge60} months.`;
}
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="mt-2 flex flex-col gap-1.5">
        ...
        <input
          ...
          className="w-full py-3 px-5 rounded-full bg-white text-slate-900 font-extrabold text-sm shadow-[inset_0_2px_4px_rgba(0,0,0,0.08),0_2px_4px_rgba(0,0,0,0.04)] border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-400"
        />

        {birthday && monthsUntilAge60 !== null && monthsUntilAge60 > 0 && (
          <div className="text-xs font-bold text-slate-600 px-2">
            Remaining repayment period until age 60: {monthsUntilAge60} months
          </div>
        )}
      </div>
